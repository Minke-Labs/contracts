...
contract MinkeMStable is ERC2771Context {
  ...
  address private constant saveWrapperAddress =
  ...
  /**
   * @dev Mints an mAsset and then deposits to Save/Savings Vault
  ...
  function saveViaMint(
      ...
      _approveToken(_bAsset, saveWrapperAddress, _amount);
      SaveWrapper(saveWrapperAddress).saveViaMint(
          _mAsset,
          _save,
          _vault,
          _bAsset,
      ...
      IERC20 _vaultToken = IERC20(_vault);
      _vaultToken.safeTransfer(_msgSender(), _vaultToken.balanceOf(address(this)));
  }
  ...
  {
    return Vault(vaultAddress).withdrawAndUnwrap(_amount, _minAmountOut, _output, _beneficiary, _router, _isBassetOut);
  }
}